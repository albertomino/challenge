    ...
    for packet in capture.sniff_continuously(packet_count=50):
        if packet.transport_layer == 'TCP':
            ...
            if ip_version == 4:
                ...
                ip = packet.ipv6
            packets_list.append({'Packet' : i, 'Packet length' : packet.length, str('sniff_time') : packet.sniff_time, 'sniff_timestamp' : packet.sniff_timestamp, 'Source IP' : ip.src, 'Source Port' : packet.tcp.srcport, 'Destination IP' : ip.dst, 'Destination port' : packet.tcp.dstport})
        i += 1
